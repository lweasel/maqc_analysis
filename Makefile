...
RANK_CORRELATIONS=$(ANALYSIS_DIR)/rank_correlations.csv
GET_QUANTIFICATION_RANKS=.get_quantification_ranks

.PHONY: all clean

all: $(ESTIMATED_READ_DEPTHS) $(RANK_CORRELATIONS) $(GET_QUANTIFICATION_RANKS)

$(GET_QUANTIFICATION_RANKS): $(CALCULATE_GENE_EXPRESSION) $(TAQMAN_GENE_ABUNDANCES)
	$(BIN_DIR)/get_quantification_ranks.sh
	touch $@

$(RANK_CORRELATIONS): $(CALCULATE_GENE_EXPRESSION) $(TAQMAN_GENE_ABUNDANCES)
	mkdir -p $(ANALYSIS_DIR)
	$(BIN_DIR)/calculate_correlations.sh > $@
